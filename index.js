...
function makePersonObject(a, b, c) {
  ...
  let person = makePersonObject(5,'Leia','leia@leia.com');
  // console.log(person);
...
*/
// let person = makePersonObject(5,'Leia','leia@leia.com');
function getName(getN) {
  return `Hello, my name is ${getN.name}`;
}
...
function getCarInfoByIndex(inventory, index) {
  const veh = inventory[index];
  return `This is a ${veh.car_make} ${veh.car_model}`
}
...
function getLastCarInfo(inventory) {
  const lastVeh = inventory[inventory.length - 1];
  return `This is a ${lastVeh.car_make} ${lastVeh.car_model}`;
}
...
*/
const sum = (a,b) => a+b;
const addFive = (ad) => ad+5;
const argTimesTwo = (timTwo) => timTwo *2; 
